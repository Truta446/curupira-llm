...
- `scripts/chat.py`: modo interativo (continuação de texto em tempo real via `GPT.stream`, comandos `/mais`, `/temp`, `/topk`, `/tamanho`, `/semente`, `/modelos`, `/modelo`, `/config`, `/ajuda`, `/sair`). Padrão: `bpe1024_rope_best.pt`, senão `best.pt`. O usuário perguntou se podia conversar com o modelo e pedir um resumo de Dom Casmurro: ele **não** é um chatbot (sem etapa de instrução, janela de 128 tokens, 1M parâmetros); deixar isso claro sempre que o assunto voltar.
